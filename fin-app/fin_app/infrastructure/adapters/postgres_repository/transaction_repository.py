...
from fin_app.infrastructure.adapters.postgres_repository.mappers import (
    TransferFromListMapper,
    TransferGetAllMapper,
    TransferMapper,
)
...
class TransactionRepository(TransactionRepositoryInterface):
    ...
    def get_by_account_id(self, id, account: AccountDomain) -> List[TransferDomain]:
        ...
        transactions = (
            Transaction.objects.select_related("transfer").filter(account_id=id).all()
        )
        for tr in transactions:
            transaction_transfer.append(
                TransferMapper.to_entity(transfer=tr.transfer, transaction=tr)
            )
        return transaction_transfer

    def transfer(self, transfer_domain: TransferDomain) -> TransferDomain:
        transfer = Transfer(id=transfer_domain.id, value=transfer_domain.value)
    ...
    def get_by_transfer_id(self, id: Decimal) -> TransferDomain:
        transactions = (
            Transaction.objects.select_related("transfer").filter(transfer_id=id).all()
        )
    ...
    def cancel_transfer(self, transfer_domain: TransferDomain):
        ...
        Transfer.objects.filter(id=transfer_domain.id).update(cancel_at=datetime.now())
